...
author: "Michelle Visscher and Kenneth Rosal 54668165"
date: "2022-08-29"
output:
...
Removing row 68 - outlier with height 364.00, shoe size 88
...
footlength_height_comb<-rbind(mergeddataFL1,footlength2.1)
footlength_height_comb #our final combined dataset
```
...
graph1 <-graphQ3+geom_histogram(fill= "#42f5b3" ) +
...
```{r}
count(footlength_height_comb, gender)
```
...
Based on our analysis of the residuals, the majority of the data does fall between +-2 Standard Deviations of the mean, however the data does not meet the assumption of linearity in that not all the residuals are distributed within +- 2 Standard Deviations. 
